...
class HotelController {
    ...
    async getHotelRooms(req, res) {
        try {
            ...
            const conflictingBookings = await this.bookingRepository.find({
                where: {
                    hotel_id: id,
                    status: (0, typeorm_1.In)([Booking_1.BookingStatus.CONFIRMED, Booking_1.BookingStatus.PENDING_PAYMENT]),
        ...
        try {
            ...
            if (owner_id) {
                const owner = await this.userRepository.findOne({
                    where: { id: owner_id, role: User_1.UserRole.HOTEL_OWNER }
                });
                if (!owner) {
                    res.status(400).json({
                        success: false,
    ...
    async deleteHotel(req, res) {
        try {
            ...
            const activeBookings = hotel.bookings?.filter(booking => [Booking_1.BookingStatus.CONFIRMED, Booking_1.BookingStatus.PENDING_PAYMENT].includes(booking.status)) || [];
            ...
            await this.hotelRepository.update(id, { status: Hotel_1.HotelStatus.INACTIVE });
            res.status(200).json({
                success: true,
                ...
                data: {
                    id: hotel.id,
                    name: hotel.name,
                    status: Hotel_1.HotelStatus.INACTIVE
                }
            });
        }
        catch (error) {
    ...
    async deleteRoom(req, res) {
        try {
            ...
            const activeBookings = room.bookings?.filter(booking => [Booking_1.BookingStatus.CONFIRMED, Booking_1.BookingStatus.PENDING_PAYMENT].includes(booking.status)) || [];
            if (activeBookings.length > 0) {
                res.status(422).json({
                    success: false,
